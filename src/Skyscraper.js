...
function Skyscraper() {
    ...
    useEffect(() => {
        ...
        function updateCameraPosition() {
            camera.position.x = buildingCenter.x + radius * Math.sin(currentPhi) * Math.sin(currentTheta);
            camera.position.y = buildingCenter.y + radius * Math.cos(currentPhi);
            camera.position.z = buildingCenter.z + radius * Math.sin(currentPhi) * Math.cos(currentTheta);
        ...
        function onMouseMove(event) {
            if (isMouseDown) {
                ...
                currentPhi = Math.max(0.8, Math.min(Math.PI - 1.4, currentPhi - deltaY * 0.005)); // Inverted and clamped to match expected direction
                ...
                if (intersects.length > 0) {
                    ...
                    if (hovered.userData.apartmentNumber !== selectedBox) { // Ensure the selected apartment is not affected by hover
    ...
    }, [theta, phi, selectedBox]); // Re-run effect if theta, phi, or selectedBox changes
